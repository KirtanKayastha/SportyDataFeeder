...
- `app/services/simulation.py` — stat-weighted live simulation as an asyncio background task: `POST /simulate` returns 202, the loop Bernoulli-samples per-player per-minute probabilities from `app.state.event_rates` (league-average fallback for cold starts), writes events with UUID `event_id` idempotency keys, pushes ONE HTTP batch per minute, sleeps `SIMULATION_SPEED` per minute (0 = max speed). Match-scoped state registry (`_simulations`) keeps concurrent simulations independent; duplicate `/simulate` for a running match → 409. On finish: ratings (R-3.5) stored in `player_match_ratings` and pushed. Pushes are skipped (with a WARNING) when the match has no `entity_links` `sporty_match_id` mapping — use `/links` then `/matches/{id}/replay-push`. Substitutions & discipline: each side carries a bench (`BENCH_SIZE`); football gets up to 5 permanent subs weighted to the 2nd half (keeper/featured players never subbed off) and a second yellow to the same player emits a red card and sends the player off (team plays short); basketball rotates 1-2 players per team every `BASKETBALL_ROTATION_INTERVAL` minutes (players return). "substitution" events carry `related_player_id` (player off) → pushed as `related_sporty_player_id`; the Sporty backend publishes a `LINEUP_CHANGE` WS message per substitution. Per-player real minutes go in the ratings payload. Football also gets: **injuries** (per-match counts pre-drawn at kickoff from capped distributions — never per-minute rolls; `injury` events carry `extra.severity`, a forced-off consumes a sub with `extra.reason="injury"` or leaves the team a man down; a minor-knocked player is preferred off at the next tactical sub window), **penalties** (`PENALTY_COUNT_DIST` ≈0.3/match; scored → `goal` with `extra.penalty`, otherwise `penalty_missed` (+`penalty_saved` for the keeper on a save); expected penalty goals are netted out of the calibration targets), **possession** (per-minute AR(1) home-share anchored on pre-calibration goal rates, modulates goal probability by share/anchor so calibration holds, nudged by goals/red cards; exposed as `possession_*_pct` on status/push payloads and persisted as a player-less `possession` event that is never pushed), and **knockout resolution** (`Match.knockout` → tie after 90' plays 30' extra time then a shootout: `shootout_goal`/`shootout_miss` events, tally in `state.shootout_*` and the push payload's `shootout` object; regulation score untouched).
...
- **Alembic is the source of truth for schema.** `main.py` does NOT call `create_all`. Migrations: `0001_initial_schema` (original 5 tables), `0002_entity_links_and_stats` (`entity_links`, `player_stats`, `match_predictions`, `player_match_ratings`), `0003_add_event_id`, `0004_add_match_knockout` (`matches.knockout` for extra-time/shootout resolution). For a DB that predates Alembic, run `alembic stamp 0001_initial_schema` once, then `alembic upgrade head`.
